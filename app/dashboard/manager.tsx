...
// Definisikan tipe data untuk Analytics dan Transactions
type Analytic = {
  analytic_id: number;
  product_id: number;
...
export default function ManagerDashboard() {
  // Tambahkan tipe data pada useState
  const [analytics, setAnalytics] = useState<Analytic[]>([]);
  const [transactions, setTransactions] = useState<Transaction[]>([]);

  // Mengambil data analytic
  useEffect(() => {
  ...
  // Fungsi untuk melihat detail analytic
  const handleViewAnalytic = async (analyticId: number) => {
    ...
    Alert.alert('Analytic Detail', JSON.stringify(data, null, 2)); // Menampilkan data analitik secara detail
  };
  ...
  const handleViewTransaction = async (transactionId: number) => {
    ...
    Alert.alert('Transaction Detail', JSON.stringify(data, null, 2)); // Menampilkan data transaksi secara detail
  };
